...
8. If `<BABA_SOURCE_SCOPE>` contains more than one corpus (`default`, `all`, or a `+`-joined combination), run one bounded `connections` search early using the user's question or a focused comparison. Look for possible cross-corpus relationships, themes, parallels, qualifications, or contrasts even when the user did not explicitly ask for a comparison. If the graph returns nothing relevant, continue with ordinary search.
9. Treat a search snippet as a locator, not automatically as complete evidence. If it contains `...`, appears truncated or ambiguous, or the answer depends on qualification, negation, attribution, surrounding context, or exact wording, call the passage operation with the returned citation before answering. Retrieve the selected passage and nearby context, and do not rely on a truncated snippet when the surrounding wording could change the meaning.
10. Use the knowledge graph for questions that require synthesis across multiple claims or source documents, especially when the user asks for a cross-source comparison, conceptual parallels, shared themes, whether one teaching supports, qualifies, or contrasts with another, or possible contradictions, tensions, exceptions, or unresolved differences. For multi-corpus scopes, the bounded check in step 8 is required; otherwise use the graph when the question requires it. Run:
...
12. The absence of a graph relationship is not proof that no relationship, contradiction, or relevant passage exists. The graph is finite and may miss a connection. If it returns nothing or the evidence is weak, continue with targeted ordinary searches and state that the selected corpus did not establish the answer when appropriate.
13. Synthesize corroborating or contrasting passages instead of selecting only the top-ranked result.
14. Inspect the returned provenance before answering.
15. Cite the discourse title plus paragraph anchor, the story book title, or the selected book category and section anchor. Do not expose PDF page numbers in answers.
16. When evidence is weak or absent, state that the selected corpus did not establish the answer.
